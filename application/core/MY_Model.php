...
class MY_Model extends CI_Model
{
     ...
     * If value is set as null, the $fillable property will be set as an array with all the table fields (except the
     * primary key) as elements. If value is set as an array, there won't be any changes done to it (ie: no field of
     * the table will be updated or inserted).
     */
     ...
     * If value is set as an array, there won't be any changes done to it (if set as empty array, the primary key won't
     * be inserted here).
     */
    ...
    public function __construct()
    {
        ...
        $this->_fetch_table();
        $this->pagination_delimiters = (isset($this->pagination_delimiters))
    ...
    {
        $this->_database->join($this->_relationships[$requested['request']]['foreign_table'],
            $this->table . '.' . $this->_relationships[$requested['request']]['local_key'] . ' = ' . $this->_relationships[$requested['request']]['foreign_table'] . '.' . $this->_relationships[$requested['request']]['foreign_key']);
        $the_select = '';
        if (!empty($requested['parameters'])) {
            if (array_key_exists('fields', $requested['parameters'])) {
                ...
                foreach ($fields as $field) {
                    $sub_select[] = ((strpos($field,
                                '.') === false) ? '`' . $this->_relationships[$requested['request']]['foreign_table'] . '`.`' . trim($field) . '`': trim($field)) . ' AS ' . $requested['request'] . '_' . trim($field);
                }
    ...
    protected function join_temporary_results($data)
    {
        foreach ($this->_requested as $requested_key => $request) {
            ...
            if (!isset($pivot_table)) {
                ...
                if (!empty($request['parameters'])) {
                    if (array_key_exists('fields', $request['parameters'])) {
                        if ($request['parameters']['fields'] == '*count*') {
                            ...
                            foreach ($fields as $field) {
                                $select[] = (strpos($field,
                                        '.') === false) ? '`' . $foreign_table . '`.`' . trim($field) . '`': trim($field);
                            }
                    ...
                    if (array_key_exists('fields',
                            $request['parameters']) && ($request['parameters']['fields'] == '*count*')) {
                        $sub_results->group_by('`' . $foreign_table . '`.`' . $foreign_key . '`');
                    }
                    if (array_key_exists('where', $request['parameters']) || array_key_exists('non_exclusive_where',
                            $request['parameters'])) {
                        $the_where = array_key_exists('where', $request['parameters']) ? 'where': 'non_exclusive_where';
                    }
                    $sub_results = isset($the_where) ? $sub_results->where($request['parameters'][$the_where], null,
                        null, false, false, true): $sub_results;
                ...
                $this->_database->join($pivot_table,
                    $foreign_table . '.' . $foreign_key . ' = ' . $pivot_table . '.' . $pivot_foreign_key, 'left');
                $this->_database->join($this->table,
                    $pivot_table . '.' . $pivot_local_key . ' = ' . $this->table . '.' . $local_key, 'left');
                $this->_database->select($foreign_table . '.' . $foreign_key);
                $this->_database->select($pivot_table . '.' . $pivot_local_key);
                if (!empty($request['parameters'])) {
                    if (array_key_exists('fields', $request['parameters'])) {
                        if ($request['parameters']['fields'] == '*count*') {
                            $this->_database->select('COUNT(`' . $foreign_table . '`.`' . $foreign_key . '`) as counted_rows, `' . $foreign_table . '`.`' . $foreign_key . '`',
                                false);
                        } else {
                ...
                if (!empty($order_inside_array)) {
                    $order_inside_str = '';
                    foreach ($order_inside_array as $order_by_inside) {
                        $order_inside_str .= (strpos($order_by_inside[0],
                                '.') === false) ? '`' . $foreign_table . '`.`' . $order_by_inside[0] . ' ' . $order_by_inside[1]: $order_by_inside[0] . ' ' . $order_by_inside[1];
                        $order_inside_str .= ',';
                    }
                ...
                foreach ($sub_results as $result) {
                    ...
                    if (isset($pivot_table)) {
                        ...
                        if (isset($get_relate) and $get_relate === true) {
                            $subs[$the_local_key][$the_foreign_key] = $this->{$relation['foreign_model']}->where($foreign_key,
                                $result[$foreign_key])->get();
                        } else {
    ...
    private function _has_one($request)
    {
        ...
        $this->_database->join($relation['foreign_table'],
            $relation['foreign_table'] . '.' . $relation['foreign_key'] . ' = ' . $this->table . '.' . $relation['local_key'],
            'left');
                        ...
                        if (!is_array($relation)) {
                            ...
                            if ($this->is_assoc($relation)) {
                                ...
                                if ($option == 'has_many_pivot') {
                                    $pivot_table = $relation['pivot_table'];
                                    $pivot_local_key = (array_key_exists('pivot_local_key',
                                        $relation)) ? $relation['pivot_local_key']: $this->table . '_' . $this->primary_key;
                                    $pivot_foreign_key = (array_key_exists('pivot_foreign_key',
                                        $relation)) ? $relation['pivot_foreign_key']: $foreign_table . '_' . $foreign_key;
                                    $get_relate = (array_key_exists('get_relate',
                                            $relation) && ($relation['get_relate'] === true)) ? true: false;
                                }
    ...
    public function paginate($rows_per_page, $total_rows = null, $page_number = 1)
    {
        ...
        if ($page_number == 1) {
            ...
            $this->previous_page = $this->pagination_delimiters[0] . anchor($uri_string,
                    $this->pagination_arrows[0]) . $this->pagination_delimiters[1];
        }
        ...
        if (isset($total_rows) && (ceil($total_rows / $rows_per_page) == $page_number)) {
            ...
            $this->next_page = $this->pagination_delimiters[0] . anchor($uri_string,
                    $this->pagination_arrows[1]) . $this->pagination_delimiters[1];
        }
    ...
    /**
     * this function verifies if a table name was defined. if not, it calls the _get_table_name in order to retrieve a
     * potential table name, which will be tested if exists. If all went well, the table name will be saved as
     * $this->table
     *
     * @return boolean
     */
    private function _fetch_table()
    {
        if (!isset($this->table)) {
            ...
            if (!$this->db->table_exists($this->table)) {
                show_error(
                    sprintf('While trying to figure out the table name, couldn\'t find an existing table named: <strong>"%s"</strong>.<br />You can set the table name in your model by defining the protected variable <strong>$table</strong>.',
                        $this->table),
                    500,
    ...
    // Custom MY_Model features - added 01.09.2018
     ...
     * $record_exists = $this->model_name->record_exists([ 'first_name' => 'John' ]);
     *
     * In this example, $record_exists will be true if `table_name` holds
     * one or more records that have the value 'John' in the 'first_name'
     * field.
     *
     * @param array       $conditions The SQL's WHERE condition
     * @param string|null $table      The database table to use
     *
     * @return bool
     */
    public function record_exists(array $conditions, ?string $table = null): bool
    {
        $t = $this->db->where($conditions)->count_all_results($table ?? $this->table);
     ...
     * @return bool
     */
    public function records_exist(array $conditions, ?string $table = null): bool
    {
        $t = $this->db->where($conditions)->count_all_results($table ?? $this->table);
